  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-4">
        <Card><CardContent className="p-6"><p className="text-2xl font-bold text-primary-600">{requests.sent?.length}</p><p className="text-sm text-gray-500">الطلبات المرسلة</p></CardContent></Card>
        <Card><CardContent className="p-6"><p className="text-2xl font-bold text-emerald-600">{requests.sent?.filter((r: any) => r.status === 'ACCEPTED').length}</p><p className="text-sm text-gray-500">المقبولة</p></CardContent></Card>
        <Card><CardContent className="p-6"><p className="text-2xl font-bold text-yellow-600">{requests.sent?.filter((r: any) => r.status === 'PENDING').length}</p><p className="text-sm text-gray-500">قيد الانتظار</p></CardContent></Card>
        <Card><CardContent className="p-6"><p className="text-2xl font-bold text-red-600">{requests.sent?.filter((r: any) => r.status === 'REJECTED').length}</p><p className="text-sm text-gray-500">المرفوض</p></CardContent></Card>
      </div>
